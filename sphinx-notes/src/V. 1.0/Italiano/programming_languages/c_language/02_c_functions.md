...
Proviamo a scrivere la funzione somma con delle variabili locali per andare a fare la somma di due numeri all'interno del main.  
...
Quello che siamo andati a fare all'interno del programma che abbiamo appena scritto consiste essenzialmente in due passaggi che avvengono contemporaneamente, la creazione di una variabile `int num1` e l'inizializzazione della variabile ad un valore di partenza che può essere un numero (`num1 = 3;`) oppure un'espressione (`sum = num1 + num2;`).  
...
Questo codice è sostanzialmente identico al codice che abbiamo scritto precedentemente con l'unica differenza che in questo caso non andiamo ad inizializzare le variabili a nessun valore che quindi partono da un valore di default (Assegnato dal compilatore stesso) ma andiamo a fare un'assegnazione di valori successivamente alla dichiarazione.  
...
1. Essendo l'inizio del comando verrà chiamato il main
2. Essendo che printf ha bisogno di tutti gli argomenti per essere richiamato come seconda funzione chiameremo sum per avere il risultato da passare alla funzione printf
...
Invece a livello di memoria cosa succede?  
Una volta che dal main viene chiamata la funzione sum con **argomenti** (i valori passati ad una funzione durante una chiamata vengono chiamati **argomenti**) 3 e 4 questi valori vengono salvati all'interno dei **parametri** (le variabili utilizzate all'interno della funzione per salvare il valore degli argomenti sono chiamati **parametri**) num1 e num2.  
Quest'ultime (num1 e num2) non sono altro che variabili locali loro stesse.   
Capiamo cosa succede all'interno della memoria quando andiamo a chiamare la funzione sum:
1. Una volta chiamata la funzione viene allocata della memoria nello stack per le tre variabili num1, num2 e sum e non gli viene assegnato un valore, è come se venissero create "vuote"  
2. Le variabili continuano ad esistere finché la funzione è in esecuzione in quanto stanno venendo usate
3. Una volta eseguito il return le variabili locali all'interno della funzione vengono "distrutte" cioè vengono deallocate dalla memoria e quelle celle di memoria specifiche potranno essere usate per altre chiamate senza rimanere vincolate alle variabili locali
